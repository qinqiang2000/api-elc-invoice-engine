...
public class RulePromptBuilder {
    ...
    private String systemPrompt() {
        return String.join("\n",
            ...
            "2) 允许命名空间：math, integer, double, db。",
            ...
            "   - 禁止直接对未知字符串执行 double.parseDouble(x)（防止 NumberFormatException）",
            "   - 合理分组与括号明确优先级，减少重复取值与过深嵌套，提升可读性",
            "   - 若需 DB：在 ruleExpression 顶部以局部变量承接一次 DB 结果（例如 supplier = db.queryOne(...)），严禁重复查询。",
